...
/* =====================================================
   SALE vs PURCHASE MISMATCH REPORT (ALL TABLES)
===================================================== */
  ...
  try {
    ...
    for (const ref_no of Object.keys(byRef)) {
      ...
      if (ref_no.startsWith("HOT-")) {
        const q = await db.query(
          `SELECT hotel_name, hotel_total, sar_rate FROM hotels WHERE ref_no=$1 AND is_deleted=false`,
          [ref_no]
        );
        if (q.rows.length) {
          q.rows.forEach((h, i) => {
            salesRows.push({
              item: `Hotel ${i + 1} - ${h.hotel_name || ""}`,
              sale_pkr: (Number(h.hotel_total) || 0) * (h.sar_rate || 0)
            });
          });
        }
      }
      ...
      if (ref_no.startsWith("VISA-")) {
        ...
        if (q.rows.length) {
          ...
          salesRows.push({
            item: "Visa",
            sale_pkr: Number(r.total_sar || 0) * Number(r.pkr_rate || 0)
          });
        }
      }
